    ...
    try {
      ...
      for (const [x, z] of chunks) {
        try {
          ...
          // Trees + buildings are generated by preloadOutdoorTerrain (a few seconds
          // later), by which point the model registries have loaded — so they use
          // the real models. Doing it here too would run before the models load
          // (procedural) AND duplicate what preload regenerates for these chunks.
        } catch (chunkError) {
